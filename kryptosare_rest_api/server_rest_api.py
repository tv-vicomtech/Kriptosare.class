...
def search_address():
	...
	if request.method == 'GET':
		...
		if ('key' in request.args) and ('currency' in request.args):
			...
			val2 = {"tag_optional":{"currency":cash},"custom_properties":{"error":1,"message":"Address or currency not found","result":""}}
			val = {**val,**val2}		
			return jsonify(val)
	else:
		val = server_queries.createinicialjson(request.json,tag_name)
	...
	if ((adds !="") and (len(cash)==3)):        
		...
		val2 = {"tag_optional":{"currency":cash},"custom_properties":{"error":1,"message":"Currency lenght not correct","result":""}}
		val = {**val,**val2}
	else:
		val2 = {"tag_optional":{"currency":cash},"custom_properties":{"error":1,"message":"Address not found","result":""}}
		val = {**val,**val2}
	return jsonify(val)      
...
def api_class_checker():
	...
	if request.method == 'GET':
		...
		if ('key' in request.args) and ('currency' in request.args) and ('class' in request.args) and ('confidence' in request.args):
			...
			val2 = {"tag_optional":{"actor_type":actor,"currency":cash},"custom_properties":{"confidence":aconf,"error":1,"message":"Address or currency or class or confidence not found","result":""}}
			val = {**val,**val2}
			return jsonify(val)
	else:
		val = server_queries.createinicialjson(request.json,tag_name)
	...
	try: aconf = int(aconf)
		...
		val2 = {"tag_optional":{"actor_type":actor,"currency":cash},"custom_properties":{"confidence":aconf,"error":1,"message":"Confidence value not correct","result":""}}
		val = {**val,**val2}
		return jsonify(val)
	else: 
		...
		if ((adds !="") and (len(cash) == 3) and (actor in classlist) and (aconf >=0 and aconf<=100)):
			...
			val2 = {"tag_optional":{"actor_type":actor,"currency":cash},"custom_properties":{"confidence":aconf,"error":1,"message":"Class not exist","result":""}}
			val = {**val,**val2}
		elif (len(cash)!=3):
			val2 = {"tag_optional":{"actor_type":actor,"currency":cash},"custom_properties":{"confidence":aconf,"error":1,"message":"Currency lenght not correct","result":""}}
			val = {**val,**val2}
		else:
			val2 = {"tag_optional":{"actor_type":actor,"currency":cash},"custom_properties":{"confidence":aconf,"error":1,"message":"Address not correct","result":""}}
			val = {**val,**val2}
		return jsonify(val)
...
def api_list_classes():
	...
	if request.method == 'GET':
		...
		if ('currency' in request.args):
			...
			val = {"tag_optional":{"currency":cash},"custom_properties":{"error":1,"message":"Currency not found","result":""}}
			return jsonify(val)
	else:
		val = server_queries.createinicialjson(request.json,tag_name)
		if ('tag_optional' in request.json):
			...
			val2 = {"tag_optional":{"currency":cash},"custom_properties":{"error":1,"message":"Currency not found","result":""}}
			val = {**val,**val2}
			return jsonify(val)
	if (len(cash) == 3):
...
def api_list_addresses():
	...
	if request.method == 'GET':
		...
		if ('key' in request.args) and ('currency' in request.args):
			...
			val = {"tag_optional":{"currency":cash},"custom_properties":{"error":1,"message":"Address or currency not found","result":""}}
			return jsonify(val)
	else:
		val = server_queries.createinicialjson(request.json,tag_name)
		if ('key' in request.json) and ('tag_optional' in request.json):
	...
	if ((adds!="") and (len(cash) == 3)):
		...
		val2 = {"tag_optional":{"currency":cash},"custom_properties":{"error":1,"message":"Address ot correct","result":""}}
		val = {**val,**val2}
	return jsonify(val)
...
def api_list_user_addresses():
	...
	if request.method == 'GET':
		...
		if ('user' in request.args) and ('currency' in request.args):
			...
			val = {"tag_optional":{"currency":cash},"custom_properties":{"error":1,"message":"User or currency not found","result":""}}
			return jsonify(val)
	else:
		val = server_queries.createinicialjson(request.json,tag_name)
