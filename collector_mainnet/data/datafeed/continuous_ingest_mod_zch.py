...
class BlockchainIngest:
    ...
    def write_next_blocks(self, start_block):
        next_block = blockutil_zch.hash_str(start_block)
        while next_block:
            block_json = blockutil_zch.fetch_block_json(next_block)
            next_block, block, txs = blockutil_zch.transform_json(block_json)
...
def main():
    ...
    blockutil_zch.set_blockchain_api("http://%s:%d/rest/block/" %
                                 (args.host, args.port))
    while True:
        if Path(LOCKFILE).is_file():
            ...
            if last_rs:
                ...
                print("\tHash:\t%s" % blockutil_zch.hash_str(hash_val))
                if hash_val:
